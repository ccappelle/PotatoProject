...
class Network(object):
	def __init__(self,num_sensors,num_hidden,num_motors):
	...
	def Send_To_Simulator(self,sim,sensor_indices=-1,motor_indices=-1,neuron_offset=0,sensor_offset=0,joint_offset=0):
		if sensor_indices < 0:
		...
		for i in range(self.total_neurons):
			for j in range(self.total_neurons):
				if not(self.adj_matrix[i,j]==0):
					sim.Send_Synapse(sourceNeuronIndex=neuron_offset+i, targetNeuronIndex=neuron_offset+j,weight=self.adj_matrix[i,j])


class LayeredNetwork(Network):

	def __init__(self,num_sensors=1,num_layers=1, hidden_per_layer=1, num_motors=1,back_connections=0,hidden_recurrence=0,motor_recurrence=0):
		...
		super(LayeredNetwork,self).__init__(num_sensors=num_sensors,num_hidden=num_layers*hidden_per_layer,num_motors=num_motors)
